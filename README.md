...
| 十八 | 爱莲说 | 周敦颐 | 宋 | 说 |
| 十九 | 记承天寺夜游 | 苏轼 | 宋 | 记 |
| 二十 | 定风波 · 莫听穿林打叶声 | 苏轼 | 宋 | 词 |
| 二十一 | 前赤壁赋 | 苏轼 | 宋 | 赋 |
| 二十二 | 三字经 · 节选 | 王应麟 | 宋 | 蒙学 |
| 二十三 | 增广贤文 · 节选 | 佚名（明清民间汇编） | 明 | 蒙学 |
...
- **段内译文**：每段白话译文可独立开关。
- **背诵模式**（遮字填空）：点击工具条「背诵」后，正文逐字渲染，所有汉字隐藏为朱砂下划线 `＿`，标点 / 数字 / 字母始终显示。点击单字即可揭示原字（再点收回）；背诵态下额外提供「全显 / 全隐」一键切换。进入背诵模式自动隐藏夹注与译文，避免泄底；切换篇目或退出背诵会清空填空进度。`reciteMode` 偏好持久化于 `localStorage`。
- **朗读**：调用系统语音合成，朗读整篇原文。
